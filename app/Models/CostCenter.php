...
{
    use HasFactory, RecordsAuditTrail;
    ...
    {
        // Compute all preceding abbreviations so collision resolution stays deterministic.
        $costCenters = self::query()->orderBy('id')->get(['id', 'name']);

        $stopWords = ['DE', 'Y', 'LA', 'EL', 'LOS', 'LAS', 'CON', 'PARA', 'POR', 'EN'];
        $seen = [];

        foreach ($costCenters as $costCenter) {
            $cleanName = Str::upper(Str::ascii((string) $costCenter->name));
            $cleanName = (string) preg_replace('/[^A-Z0-9]+/', ' ', $cleanName);
            $words = preg_split('/\s+/', trim($cleanName), -1, PREG_SPLIT_NO_EMPTY) ?: [];
            ...
            if (isset($seen[$baseAbbr])) {
                $baseAbbr .= $costCenter->id;
            }

            $seen[$baseAbbr] = true;

            if ($costCenter->id === $this->id) {
                return $baseAbbr;
            }
        }
