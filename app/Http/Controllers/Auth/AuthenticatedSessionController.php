...
class AuthenticatedSessionController extends Controller
{
    ...
    public function redirectToGoogle(Request $request): RedirectResponse
    {
        ...
        if (blank($config['client_id']) || blank($config['client_secret']) || blank($config['redirect'])) {
            return redirect()->route('login')->withErrors([
                'email' => 'El acceso con Google aún no está configurado.',
            ]);
        }
    ...
    public function handleGoogleCallback(
        ...
        if (blank($expectedState) || ! hash_equals($expectedState, (string) $request->query('state'))) {
            return redirect()->route('login')->withErrors(['email' => 'La sesión de Google expiró. Intenta nuevamente.']);
        }

        if ($request->filled('error')) {
            return redirect()->route('login')->withErrors([
                'email' => $request->query('error_description', 'No fue posible iniciar sesión con Google.'),
            ]);
        }

        if (! $request->filled('code')) {
            return redirect()->route('login')->withErrors(['email' => 'Google no devolvió un código de acceso válido.']);
        }
        ...
        try {
            ...
            return redirect()->route('login')->withErrors([
                'email' => 'No fue posible validar tu cuenta de Google. Intenta nuevamente.',
            ]);
        }

        $identityEmail = strtolower(trim((string) ($identity['email'] ?? '')));
        if (! filter_var($identityEmail, FILTER_VALIDATE_EMAIL) || ! $this->isAuthorizedGoogleEmail($identityEmail)) {
            return redirect()->route('login')->withErrors([
    ...
    {
        return $this->isEmailDomainAllowed($email, config('services.microsoft.allowed_domains', []));
    }

    private function isAuthorizedGoogleEmail(string $email): bool
    {
        return $this->isEmailDomainAllowed($email, config('services.google.allowed_domains', []));
    }

    private function isEmailDomainAllowed(string $email, array $allowedDomains): bool
    {
