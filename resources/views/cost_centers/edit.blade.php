                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('cost_centers.update', $costCenter->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" value="{{ $costCenter->name }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="director_id" id="director_id" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                ...
                                @foreach($directors as $director)
                                    <option value="{{ $director->id }}" {{ $costCenter->director_id === $director->id ? 'selected' : '' }}>{{ $director->name }}</option>
                                @endforeach
                            </select>
