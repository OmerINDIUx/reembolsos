                    ...
                    @if($exactSearchResult)
                        @php
                            $exactStage = match($exactSearchResult->status) {
                                'enviado' => $exactSearchResult->currentStep?->name ?? 'Enviado',
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            <div class="flex flex-col space-y-1">
                                                <span class="px-2 w-fit inline-flex text-[10px] leading-5 font-black uppercase rounded-full 
                                                    ...
                                                    {{ $r->status === 'enviado' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' : '' }}
                                                    ...
                                                    {{ !in_array($r->status, ['aprobado', 'rechazado', 'requiere_correccion', 'enviado', 'pendiente_revision_cxp', 'pendiente_pago', 'borrador']) ? 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' : '' }}
                                                ">
                                                    @if($r->status === 'aprobado') Pago aprobado
                                                    @elseif($r->status === 'enviado') {{ $r->currentStep->name ?? 'Enviado' }}
                                                ...
                                                <span class="text-[10px] text-gray-400 font-medium italic">
                                                    @if($r->status === 'enviado' && $r->currentStep)
