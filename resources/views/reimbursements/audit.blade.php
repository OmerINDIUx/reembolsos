        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($auditItems && $auditMeta)
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-3xl border border-gray-100 dark:border-gray-700">
                    ...
                    {{-- Header de Auditoría --}}
                    <div class="p-8 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end border-b border-gray-100 dark:border-gray-700 bg-indigo-50/50 dark:bg-indigo-900/10">
                        <div class="space-y-2">
                            <div class="flex items-center space-x-2">
                                <span class="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest italic opacity-60">Auditoría de Reembolsos</span>
                                <div class="h-px w-8 bg-indigo-200 dark:bg-indigo-700"></div>
                            </div>
                            <h3 class="text-3xl font-black text-gray-900 dark:text-gray-100 uppercase tracking-tight leading-none">
                                {{ $auditMeta['cc_name'] }}
                            </h3>
                            <div class="flex items-center space-x-3 mt-2">
                                <span class="px-3 py-1 bg-indigo-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest">Semana {{ $auditMeta['week'] }}</span>
                                <span class="px-3 py-1 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded-lg text-[9px] font-black uppercase tracking-widest border border-gray-200 dark:border-gray-600">{{ ucfirst(str_replace('_', ' ', $auditMeta['type'])) }}</span>
                            </div>
                        </div>
                        <div class="mt-6 md:mt-0 p-6 bg-white dark:bg-gray-800 rounded-3xl border border-indigo-100 dark:border-indigo-800 shadow-xl flex flex-col items-start md:items-end min-w-[280px]">
                            <span class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Monto Total Auditado</span>
                            <span class="text-4xl font-black text-indigo-700 dark:text-indigo-400 tracking-tighter leading-none">${{ number_format($auditMeta['total'], 2) }}</span>
                            <span class="text-[10px] text-gray-400 font-bold mt-2 italic">* Acumulado de {{ $auditMeta['count'] }} facturas validadas</span>
                        </div>
                    </div>
            ...
            @else
                ...
                <div class="bg-gray-50 dark:bg-gray-900/40 rounded-[2.5rem] p-4 md:p-8 border border-gray-100 dark:border-gray-800">
                    ...
                    @if($selectedWeek)
                        <div class="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end p-2">
                            <div>
                            ...
                            @php 
                                ...
                                $displaySum = $displayItems->sum('total');
                            @endphp
                            <div class="mt-6 md:mt-0 bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl min-w-[250px]">
                                <span class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1 block">Acumulado del Periodo</span>
                                <span class="text-4xl font-black text-indigo-600 dark:text-indigo-400 tracking-tighter leading-none">${{ number_format($displaySum, 2) }}</span>
                            </div>
                        </div>
